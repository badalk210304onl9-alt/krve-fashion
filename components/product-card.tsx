...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="product-card exact-product-card">
      <div className="product-image-wrap">
        ...
        <Link href={`/product/${product.slug}`}>
          <Image src={product.image} alt={product.name} fill sizes="(max-width: 700px) 100vw, 24vw" className="product-image" />
        </Link>
      </div>
      <div className="product-info">
        ...
        <button className="quick-add" onClick={() => addToCart(product)}>ADD TO BAG</button>
      </div>
    </article>
  );
}
